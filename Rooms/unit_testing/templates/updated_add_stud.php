...
$new_status = $row["occupancy_status"]+1;
$file_pointer = fopen($filename, "a");
if($file_pointer == false)
{
	echo "Could not open file.\nLogging not done";
}
fwrite($file_pointer, date('M d, H:i:s')." ADD: $user added student ".$_POST["stud"]." to room no. ".$row["room_no"].".\n".date('M d, H:i:s')." STATUS: Occupancy status of room no. ".$row["room_no"]." changed to ".$new_status.".\n");
fclose($filename);
...
$file_pointer = fopen($filename, "a");
fwrite($file_pointer, date('M d, H:i:s')." ADD ERROR: $user could not add ".$_POST["stud"]." to room ".$row["room_no"].". Student already exists in room ".$error_row["room_no"].".\n");
fclose($filename);
?>
			<div class="full_width big">
