...
$result = mysql_query("SELECT * FROM students s,complaints c,student_room r where s.roll_no=c.student_id and (s.roll_no=r.student1 or s.roll_no=r.student2 or s.roll_no=r.student3) order by c.status,c.complaint_date,c.complaint_time desc");
//echo "a".$result."b";
?>
...
?>
  ...
  <tr>
      ...
      <td height="23" colspan="6" valign="top"> <a href="close.php?idd=<?php echo $row['complain_id'] ?>">Close Complaint</a> </td>
            <?php }else{ ?>
    <td height="23" colspan="6" valign="top">Complaint Resolved<br></br></td>
            <?php } ?>

     <?php }else{ ?>
            <?php if($row['status']=="False"){ ?>
    <td height="23" colspan="6" valign="top">Complaint Unresolved<br></br></td>
            <?php }else{ ?>
